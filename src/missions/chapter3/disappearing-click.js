...
export default {
  ...
  runtimeExplanation: '`addEventListener`, `event.target`/`currentTarget`, and `closest()` are all DOM APIs, not ECMAScript — the language has no concept of elements or event dispatch at all; this behavior is defined by the DOM Standard. `event.target` is fixed for the lifetime of a given dispatched event: it\'s always the actual originating element, set once at dispatch time, and does not change as the event propagates through capturing, target, and bubbling phases. `event.currentTarget` changes as the event moves through the propagation path — it\'s always whichever element the currently-executing listener is attached to. A common gotcha: `currentTarget` is reset to `null` once dispatch finishes, so reading it later (e.g. inside a `setTimeout` scheduled from the handler) will not give you the element — capture it into a local variable first if you need it after the handler returns. `closest(selector)` walks from the element itself up through the `parentElement` chain, returning the nearest ancestor-or-self matching the selector, or `null` if none match.',
