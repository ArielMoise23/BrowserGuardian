...
scheduleSessionWarnings(3);`,
  ...
  runtimeExplanation: '`var` declarations are function-scoped and hoisted to the top of their enclosing function; a `for (var i ...)` loop does not get a new `i` per iteration — there is one `i`, mutated in place, for the whole loop. `let`/`const` are block-scoped, and the specification defines `for (let i ...)` as creating a fresh lexical environment for each iteration — critically, each new environment is initialized by *copying the value* of `i` from the previous iteration, not by resetting it to the loop\'s initial value. That copy-forward step is what people usually miss: it\'s why the loop still counts up normally (0, 1, 2, ...) even though every iteration technically has its own independent `i` binding. This is scope behavior defined by the ECMAScript specification (CreatePerIterationEnvironment) — not a browser quirk, and identical in Node.js, browsers, or any other conformant engine.',
