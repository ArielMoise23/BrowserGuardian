...
export default {
  ...
  runtimeExplanation: 'An `<img>` element\'s `src` setter triggers a real browser-level GET request the instant it\'s assigned — this bypasses `fetch`/XHR entirely, so it is a "simple" cross-origin request that never triggers a CORS preflight, and no response ever needs to be read (the attacker only needs the request to hit their server\'s access logs). That combination — a different API, no preflight, no readable response — is exactly why it\'s a favorite low-friction exfiltration channel, and exactly why a monitor that only patches `fetch` and `XMLHttpRequest` (a very common first pass) would miss it completely.',
