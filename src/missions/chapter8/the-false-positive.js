...
window.__imagePolicy = function (url) {
  ...
  runtimeExplanation: '`URL` is a Web API (the WHATWG URL Standard), not an ECMAScript built-in — but it\'s exposed as a global in browsers, Node.js, and Workers alike. Constructing one and reading `.hostname` gives you the platform\'s own authoritative parse of the URL\'s host component, correctly handling edge cases (userinfo, ports, IDN, percent-encoded characters) that a naive string match gets wrong — this is the standard, spec-correct way to answer "what host is this URL actually pointing at."',
