...
export default {
  ...
  prerequisites: 'Chapter 1, and a basic idea that some things run "later" than others in JavaScript.',
  ...
  securityImpact: 'Ordering assumptions like this are exactly where race conditions in runtime security instrumentation come from: if a detector reads a value that a "later" (but actually earlier, because it\'s a microtask) piece of code hasn\'t set yet — or vice versa — you get a false negative or a crash, not a warning.',
  runtimeExplanation: 'This ordering (sync → drain full microtask queue → one macrotask → drain microtask queue again → repeat) is defined by the HTML/ECMAScript event loop model, not a browser-specific quirk — Promise reactions and queueMicrotask are both explicitly microtasks; setTimeout callbacks are explicitly macrotasks (a "task" in spec terms). The relative priority (microtasks always fully drain between tasks) is guaranteed, not implementation-dependent.',
