...
export default {
  ...
  explanation: [
    'A click does not just happen "at" the element you clicked — the browser already knows the full ancestor chain from the document root down to that element before dispatch even starts, and it walks that chain twice: once inward (capturing), once outward (bubbling), calling whichever listeners are registered for each direction. Delegation, capture-phase interception, and the target/currentTarget distinction are all consequences of one fact: the event visits every ancestor, in and then back out.',
    'Dispatch happens in three phases: capturing (root down to, but not including, the target — only `{capture: true}` listeners run here), target (listeners on the target run in registration order regardless of their capture flag), then bubbling (target back up to root — only non-capturing listeners run here, unless the event\'s `bubbles` property is false, which skips this phase entirely). `event.target` is fixed for the whole dispatch — the real originating element. `event.currentTarget` changes throughout — whichever element the currently-executing listener is attached to.',
    'One case worth knowing: `cloneNode(true)` copies an element\'s attributes, children, and structure, but never its event listeners, because listeners are not part of the DOM tree — the browser tracks them separately. Replacing a node with its clone (`node.replaceWith(node.cloneNode(true))`) is a real way, accidental or deliberate, to silently strip every listener from an element without changing its visible markup. A capture-phase listener on a stable ancestor (delegation) is immune to this, since it never depended on the replaced node having its own listener.',
    'This has a direct security angle: `document.addEventListener("input", harvest, true)` is both the standard way to build defensive instrumentation and the standard way a malicious script skims a payment form. One capture-phase listener on `document` needs no knowledge of the page\'s structure and sees every keystroke before the page\'s own bubble-phase handlers do, including any client-side masking or validation — nothing about the registration syntax tells the two apart, only who registered it and what it does with the data. Capture-phase advantage also only holds for whichever listener was attached FIRST, since same-element, same-phase listeners run in registration order, so a monitor needs to be one of the earliest scripts on the page to reliably observe before untrusted code does.',
  ],

  distinctions: [
    { label: 'Browser-provided', text: 'The entire capture/target/bubble dispatch model is defined by the DOM Standard, not ECMAScript — a plain Node.js script with no DOM has no such concept at all.' },
    { label: 'Simplified model', text: '"Bubbling" as a rising-bubble metaphor is a helpful name, not a mechanism — it is a defined traversal of the already-computed ancestor path back toward the root, calling non-capturing listeners along the way.' },
    { label: 'Implementation detail', text: 'How a browser internally stores and iterates a node\'s listener list is unspecified — the only observable guarantee is that same-element, same-phase listeners run in registration order, and capture always precedes target which always precedes bubble.' },
  ],
  ...
  tldr: [
    ...
    'event.target is fixed for the whole dispatch; event.currentTarget is whichever element the currently-running listener is attached to, and changes as propagation moves.',
    'cloneNode(true) never copies event listeners — replacing a node with its clone silently strips every listener from it.',
    'A capture-phase listener on document needs no knowledge of page structure to observe every interaction below it — the same mechanism defensive instrumentation and form-skimming malware both rely on.',
  ],
