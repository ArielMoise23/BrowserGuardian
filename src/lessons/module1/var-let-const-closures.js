...
queueToasts(["Saved", "Synced", "Done"]);`,
  ...
  knowledgeCheck: [
    { id: 'kc-hoisting', question: 'Explain the difference between how `var` and `let` behave when accessed before their declaration line runs.', modelAnswer: '`var` is hoisted and initialized to `undefined` immediately, so accessing it early gives `undefined`, not an error. `let`/`const` are also hoisted (in the sense that the engine knows about the binding from the start of the block) but are left uninitialized — in the "Temporal Dead Zone" — until their declaration statement actually executes; accessing them before that point throws a `ReferenceError`, not just returns `undefined`.', skillTag: 'fundamentals' },
