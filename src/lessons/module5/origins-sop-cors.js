...
export default {
  ...
  explanation: [
    'Two URLs are either the same origin or they are not — scheme, host, and port must all match exactly, no partial credit. Almost everything the browser does to keep one website from reading or interfering with another — blocking DOM access, blocking response reads, scoping cookies — comes down to asking whether the accessing code\'s origin matches the target\'s. CORS does not change that boundary; it is a narrow, server-controlled exception a site can opt into for specific cross-origin reads.',
    'An origin is exactly the triple {scheme, host, port} — `https://shop.example.com` and `http://shop.example.com` are different origins because the scheme differs, even with an identical host. "Site" is a coarser, different concept: the registrable domain plus scheme, so `shop.example.com` and `blog.example.com` are different origins but the same site. The Same-Origin Policy is the browser\'s default rule that script on one origin cannot read the DOM or most responses belonging to a different origin. CORS lets a SERVER opt back in to allowing a specific cross-origin response to be read — it relaxes SOP for reads on the server\'s say-so; a missing CORS header does not stop the request from being sent, only from being readable by the requesting page\'s JavaScript.',
    'Not every cross-origin request is "simple." A request only skips the preflight if it uses GET/HEAD/POST with a small allowlisted set of headers and a restricted Content-Type; anything else — a custom `Authorization` header, `Content-Type: application/json`, a PUT/DELETE method — makes the browser send an `OPTIONS` preflight first, asking the server to confirm the real request is allowed before it is sent. Credentialed requests (cookies, HTTP auth) add another layer: the server must respond with an exact `Access-Control-Allow-Origin` (never a wildcard) plus `Access-Control-Allow-Credentials: true`, or the browser refuses to expose the response — specifically to stop a permissive CORS policy from exposing cookie-authenticated data to any origin that asks.',
    'A common real vulnerability is a server that reflects whatever `Origin` header it received back as `Access-Control-Allow-Origin` instead of checking an allowlist — combined with `Access-Control-Allow-Credentials: true`, this lets an attacker\'s page fetch a victim\'s authenticated endpoint with credentials included and actually read the response back, exactly what the Same-Origin Policy exists to prevent. A runtime monitor gets no special exemption from that same policy: it cannot read a cross-origin iframe\'s DOM or network calls any more than any other script can, and the sanctioned way information crosses that boundary — `postMessage` — needs the same `event.origin` verification a monitor would expect of any application code.',
  ],

  distinctions: [
    { label: 'Browser-provided', text: 'Origins, the Same-Origin Policy, and CORS enforcement are entirely implemented by the browser at the network/DOM-access layer — no JavaScript, however clever, can grant itself access SOP would otherwise deny.' },
    { label: 'Simplified model', text: '"CORS blocks cross-origin requests" is a common but inaccurate shorthand — simple cross-origin requests are usually sent regardless; CORS controls only whether the response is readable by the requesting page\'s JavaScript.' },
    { label: 'Implementation detail', text: 'Exactly which request characteristics trigger a preflight versus count as "simple" is defined by the Fetch spec, but how a browser batches or caches preflight results is an implementation detail on top of that baseline.' },
  ],

  tldr: [
    'Origin = scheme + host + port, compared exactly. Site = registrable domain + scheme — a coarser concept used for cookies (SameSite), not the Same-Origin Policy.',
    ...
    'Custom headers, non-simple content types, or non-GET/HEAD/POST methods trigger an OPTIONS preflight before the real request is sent.',
    'Credentialed cross-origin responses require an exact Access-Control-Allow-Origin (never a wildcard) plus Access-Control-Allow-Credentials: true — reflecting the Origin header back verbatim defeats this.',
  ],
