...
export default {
  ...
  mentalModel: {
    explanation: 'JavaScript itself (the ECMAScript language) executes on a single call stack, one function call at a time, synchronously. Nothing about "waiting" or "scheduling later" is part of the language\'s execution model — that\'s all provided by the HOST ENVIRONMENT (the browser, or Node.js). When you call `setTimeout`, `fetch`, or attach a DOM event listener, you\'re calling a Web API the browser provides, not a JavaScript language feature. When that Web API\'s work is ready (a timer elapses, a response arrives, a click happens), the HOST schedules a callback to run — in HTML-standard terms, this is a "task". The event loop is a host-environment control loop that repeatedly: runs one task, then drains the ENTIRE microtask queue (which includes Promise reactions), then — in a browser — may perform rendering, then picks the next task. Microtasks always fully drain between tasks, which is why they consistently "win" over the next task even when both look "ready" at the same moment.',
    distinctions: [
